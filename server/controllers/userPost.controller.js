...
export const createPost = async (req, res) => {
  try {
    ...
    const userProfile = await Profile.findOne({ userId: authorId });
    const user = await User.findById(authorId).select("slug");
    ...
    const postsWithDetails = await Promise.all(
      posts.map(async (post) => {
        ...
        if (req.user) {
          ...
          postObj.userReaction = reaction ? reaction.type : null;
        }
        return postObj;
      })
    );

...
const userSchema = new mongoose.Schema(
  {
    ...
    dailyJobApplicationsCount: {
      type: Number,
      default: 0,
    },
    dailyJobApplicationsLimit: {
      type: Number,
      default: 3,
    },
...
// Method to reset job application limit
userSchema.methods.resetJobApplicationLimit = function () {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  ...
  if (today > lastReset) {
    this.dailyJobApplicationsCount = 0;
    this.lastJobApplicationReset = new Date();
  }
};

// Method to check if can apply for job
userSchema.methods.canApplyForJob = function () {
  if (this.isPremium()) return true;
  this.resetJobApplicationLimit();
  return this.dailyJobApplicationsCount < this.dailyJobApplicationsLimit;
};
